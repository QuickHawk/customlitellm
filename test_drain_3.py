...
if __name__ == "__main__":
    ...
    PATH = os.path.join("router_templates", "smart_routing_template_router.bin")
    PKL_PATH = os.path.join("router_templates", "smart_routing_template_router_metadata.pkl")

    router = TemplateRouter(persistence_file=PATH, metadata_file=PKL_PATH)
    
    # # List of prompts to process
    # prompts = [
    #     "Write a Python function to calculate factorial of 5",
    #     "Write a Python function to calculate factorial of 10",
    #     "Generate JavaScript code for sorting arrays",
    #     "Generate JavaScript code for filtering arrays",
    #     "Explain machine learning in simple terms",
    #     "Explain quantum computing in simple terms",
    #     "Write a Python function to calculate factorial of 100"
    # ]
    
    # # Process each prompt
    # for prompt in prompts:
    #     # Check if template exists using match
    #     existing = router.match(prompt)
        
    #     if existing:
    #         # Template found
    #         print(f"✓ MATCHED: '{prompt}'")
    #         print(f"  Template: {existing['template']}")
    #         print(f"  Cluster ID: {existing['cluster_id']}, Size: {existing['size']}\n")
    #     else:
    #         # Template not found - create new
    #         result = router.add_log_message(prompt)
    #         print(f"✗ NEW: '{prompt}'")
    #         print(f"  Template: {result['template_mined']}")
    #         print(f"  Cluster ID: {result['cluster_id']}\n")
            
    #         # Optionally set metadata for routing later
    #         if "Python" in prompt:
    #             router.set_cluster_metadata(
    #                 result['cluster_id'], 
    #                 name="python_coding",
    ...
    #             router.set_cluster_metadata(
    #                 result['cluster_id'],
    #                 name="javascript_coding", 
    #                 target_model="codellama"
    #             )

    # router.set_cluster_metadata(1, target_model="gemini/gemini-2.0-flash")
    # router.set_cluster_metadata(2, target_model="gemini/gemini-2.0-flash")
    # router.set_cluster_metadata(3, target_model="gemini/gemini-2.5-flash-lite")
    # router.set_cluster_metadata(4, target_model="gemini/gemini-2.5-flash-lite")
    # router.set_cluster_metadata(5, target_model="gemini/gemini-2.0-flash")
    # gemini/gemini-2.0-flash-lite
    
    # Print summary
    router.print_summary()
    
    # Example: Using the __call__ method (SemanticRouter-like interface)
    # route_result = router("Write a Python function to calculate factorial of 50")
    # if route_result:
    #     print(f"\nRouted to: {route_result['name']} (Cluster {route_result['cluster_id']})")
